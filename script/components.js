...
class FAQ {
    constructor() {
        this.faqItems = document.querySelectorAll('.faq');
        console.log('FAQ items found:', this.faqItems.length);
        this.init();
    }

    init() {
        this.faqItems.forEach((faq, index) => {
            const accordion = faq.querySelector('.faq-accordion');
            const content = faq.querySelector('.content');
            const icon = accordion ? accordion.querySelector('i') : null;

            console.log(`FAQ ${index}:`, { accordion, content, icon });
            ...
            if (accordion && content && icon) {
                ...
                accordion.addEventListener('click', (e) => {
                    ...
                    this.faqItems.forEach((otherFaq, otherIndex) => {
                        if (otherFaq !== faq) {
                            const otherContent = otherFaq.querySelector('.content');
                            const otherIcon = otherFaq.querySelector('.faq-accordion i');
                ...
                console.warn(`FAQ ${index} missing required elements:`, { accordion, content, icon });
            }
        });
    }
}